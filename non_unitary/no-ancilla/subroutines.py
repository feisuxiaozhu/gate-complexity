...
def test_hessian_1(rho, gradients, gateset, dt0, H):
    tolerance=1e-10
    dt = dt0
    num_qubits = len(gateset[0].dims[0])
    Hessian = compute_hessian(rho, H, gateset)
    ...
    # print(negative_indices)
    epsilon_col = [0 for _ in range(len(gradients))]
    for i in range(len(epsilon_col)):
        epsilon_col[i] = np.random.normal(0,np.sqrt(dt))
    epsilon_col = Q.T@ epsilon_col

    for i in range(len(epsilon_col)):
        if i not in negative_indices:
