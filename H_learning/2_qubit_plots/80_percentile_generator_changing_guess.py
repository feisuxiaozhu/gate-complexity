...
if __name__ == "__main__":
    for scaling in [1,5,10,15,20,30,35,40,45,50]:
        ...
        for eps, N_shots in zip( eps_col, N_shots_col):
            ...
            x0 = lambda_true-delta*scaling
            normdiff = np.linalg.norm(lambda_true-delta*scaling)
            print('norm diff: '+str(np.linalg.norm(lambda_true-delta*scaling)))
            nu= 5
            # eps = 1e-2
            # N_shots=25
            # print(eps, N_shots)
            ...
            totalT = f"{np.mean(T_all_exp):.3e}"
            print("average l2 error (unfiltered): " + f"{unfiltered_mean:.3e}")
            print("std (unfiltered): " + f"{unfiltered_std:.3e}")
            print("---------------------------------------------------")
            filename = f"l2_error_nu{nu}_eps{eps}_shots{N_shots}_scaling{scaling}_totalT{totalT}_normdiff{normdiff}.csv"
            filepath = os.path.join("data_changing_initial_guess", filename)
